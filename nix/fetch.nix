...
# string for a locked source. Every caller here wants one shape -- a
# directory to hand to callPackage -- and this is where the two become one.
# nix/wire.nix maps it over the whole set.
#
...
# already fetched. `parseFlakeRef` cannot take it: a pure evaluation
